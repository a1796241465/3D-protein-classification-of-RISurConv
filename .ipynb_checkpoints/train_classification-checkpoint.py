"""
Modified for Protein VTK Dataset Training
Author: Your Name
Date: 2024
"""
import os
...
def main(args):
    ...
    train_dataset = ProteinTrainDataset(
        vtk_dir=args.vtk_dir_train,  # 直接使用训练集VTK路径（无需os.path.join）
        csv_path=args.csv_path_train,  # 直接使用训练集CSV路径
        args=args
    )

    # 测试集 - 使用专用测试参数
    test_dataset = ProteinTestDataset(  # 注意改为ProteinTestDataset
        vtk_dir=args.vtk_dir_test,  # 直接使用测试集VTK路径
        csv_path=args.csv_path_test,  # 直接使用测试集CSV路径
        args=args
    )
