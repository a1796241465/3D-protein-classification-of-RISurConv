...
def compute_LRA_one(group_xyz, weighting=False):
    ...
    with torch.no_grad():
        ...
        if weighting:
            ...
            weights[weights != weights] = 1.0  # 处理NaN
        ...
        LRA = Vh.transpose(-2, -1)[..., 0]  # 取最小特征向量
    ...
    return LRA.detach()  # 确保返回无梯度的Tensor
...
def calculate_surface_norm(surface_norm1, surface_norm2):
    norm_x = surface_norm1[:, :, :, 1] * surface_norm2[:, :, :, 2] - surface_norm1[:, :, :, 2] * surface_norm2[:, :, :,
                                                                                                 1]
    norm_y = surface_norm1[:, :, :, 2] * surface_norm2[:, :, :, 0] - surface_norm1[:, :, :, 0] * surface_norm2[:, :, :,
                                                                                                 2]
    norm_z = surface_norm1[:, :, :, 0] * surface_norm2[:, :, :, 1] - surface_norm1[:, :, :, 1] * surface_norm2[:, :, :,
                                                                                                 0]
...
def RISP_features(xyz, norm, new_xyz, new_norm, idx, group_all=False):
    ...
    sin_angle_1_0, sin_angle_2_0, length_0 = calculate_two_surface_feature(p_point, p_norm, xi,
                                                                           xi_norm)  # alpha1 beta 1
    sin_angle_1_1, sin_angle_2_1, length_1 = calculate_two_surface_feature(p_point, p_norm, x3,
                                                                           x3_norm)  # alpha2 theta 1
...
class RISurConvSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, out_channel, group_all, protein_scale=1.0):
        super(RISurConvSetAbstraction, self).__init__()
        self.npoint = npoint
        if radius != None:
            self.radius = radius * protein_scale  # 根据蛋白质数据尺度调整半径
        self.nsample = nsample
        self.group_all = group_all

        # RI特征嵌入层（保持不变）
        self.embedding = nn.Sequential(
            nn.Conv2d(14, 32, kernel_size=1, bias=False),
            nn.BatchNorm2d(32),
            nn.Conv2d(32, 64, kernel_size=1, bias=False),
            nn.BatchNorm2d(64)
        )
        self.self_attention_0 = SA_Layer_2d(64)

        # 修改输入通道适配（关键修改点）
        self.risurconv = nn.Sequential(
            nn.Conv2d(64, out_channel, 1),  # 64(RI特征) + in_channel(电势等)
    ...
    def forward(self, xyz, norm, points):
        ...
        # 分割输入特征（关键修改点）
        if norm is not None:  # normal_channel=True时，输入为8通道
            coords = xyz[:, :, :3]  # [x,y,z]
            potentials = xyz[:, :, :2]  # [potential, normal_potential]
            normals = xyz[:, :, :3]  # [nx,ny,nz]
        else:  # normal_channel=False时，输入为5通道
            coords = xyz[:, :, :3]
            potentials = xyz[:, :, :2]
            normals = None
        ...
        # 采样和分组（保持几何处理不变）
        if self.group_all:
            new_xyz, ri_feat, new_norm, idx = sample_and_group_all(coords, normals if normals is not None else coords)
        else:
            new_xyz, ri_feat, new_norm, idx = sample_and_group(
                self.npoint, self.radius, self.nsample,
                coords,
                normals if normals is not None else coords
            )
        ...
        #     potentials = potentials.permute(0, 2, 1)

        #     print("points shape:", points.shape)
        #     print("potentials shape:", potentials.shape)

        #     grouped_points = index_points(torch.cat([points, potentials], dim=-1), idx)  # 拼接旧特征和电势
        #     grouped_points = grouped_points.permute(0, 3, 2, 1)  # [B,N,nsample,C] -> [B,C,nsample,N]
        #     grouped_points = grouped_points.reshape(B, -1, self.npoint, self.nsample)
        ...
        #     new_points = ri_feat
        # else:
        #     grouped_potentials = index_points(potentials, idx).permute(0, 3, 2, 1)
        #     new_points = torch.cat([ri_feat, grouped_potentials], dim=1)

        # 后续处理（保持不变）
        # print('points=====>',str(ri_feat.shape))
        new_points = F.relu(self.risurconv(ri_feat))
        # print("new_points形状:", new_points.shape)  # 应为 [B, C, N]
        # print("new_points最小值:", new_points.min().item())
        # print("new_points最大值:", new_points.max().item())
        risur_feat = torch.max(new_points, 2)[0]
